...
2. Create a network to use for the windows docker container.

```bash
docker network create -d macvlan --subnet=10.10.10.0/24 --gateway=10.10.10.1 --ip-range=10.10.10.0/24 -o parent=eth0 mydockernetwork-emu
```
...
4. Once the Docker Container is downloaded. The next step is to get into the Windows System in your browser and it should show the URL in docker desktop in the container itself. It should be something similar to http://127.0.0.1:8006 you can find the IP address once the windows system starts up and you can go to that URL.
